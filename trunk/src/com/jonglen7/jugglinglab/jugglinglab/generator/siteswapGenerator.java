...
import com.jonglen7.jugglinglab.jugglinglab.util.JuggleException;
import com.jonglen7.jugglinglab.jugglinglab.util.JuggleExceptionDone;
import com.jonglen7.jugglinglab.jugglinglab.util.JuggleExceptionUser;
import com.jonglen7.jugglinglab.org.apache.regexp.CharacterArrayCharacterIterator;
import com.jonglen7.jugglinglab.org.apache.regexp.RE;
import com.jonglen7.jugglinglab.org.apache.regexp.RESyntaxException;
...
public class siteswapGenerator extends Generator {
    ...
    public void initGenerator(String[] args) throws JuggleExceptionUser {
            ...
            if (i != (argc - 1))
                ...
                if ((i < (args.length - 1)) && (args[i + 1].charAt(0) != '-')) {
                    try {
                        ...
                        throw new JuggleExceptionUser(errorstrings.getString("Error_number_format_prefix")+
                                " '"+"simultaneous_throws"+"' "+errorstrings.getString("Error_number_format_suffix"));
                    }
                    i++;
                }
            }
                ...
                if ((i < (args.length - 1)) && (args[i + 1].charAt(0) != '-')) {
                    try {
                        ...
                        throw new JuggleExceptionUser(errorstrings.getString("Error_number_format_prefix")+
                                                      " '"+"Jugglers"+"' "+errorstrings.getString("Error_number_format_suffix"));
                    }
                    i++;
                }
            }
        ...
        try {
            ...
            throw new JuggleExceptionUser(errorstrings.getString("Error_number_format_prefix")+
                                          " '"+"balls"+"' "+errorstrings.getString("Error_number_format_suffix"));
        }
        try {
            ...
            throw new JuggleExceptionUser(errorstrings.getString("Error_number_format_prefix")+
                                          " '"+"max._throw"+"' "+errorstrings.getString("Error_number_format_suffix"));
        }
        try {
			if (args[2].equals("-")) {
				llow = rhythm_period;
            ...
            throw new JuggleExceptionUser(errorstrings.getString("Error_number_format_prefix")+
                                          " '"+"period"+"' "+errorstrings.getString("Error_number_format_suffix"));
        }
        ...
        if (((llow % rhythm_period) != 0) || (lhigh % rhythm_period) != 0)
            throw new JuggleExceptionUser(errorstrings.getString("Error_period_multiple_prefix")+" "+
	...
	*/
    ...
    public int runGenerator(GeneratorTarget t, int max_num, double secs) throws JuggleExceptionUser {
        ...
        /*if (numflag != 0) {
            if (num == 1)
                target.setStatus("1 pattern");
            else
                target.setStatus(num + " patterns");
        }*/
	...
	*/
    ...
    protected boolean valid_throw(int pos) {
		...
		for (i = 0; i < exclude.size(); i++) {
			...
			CharacterArrayCharacterIterator ci = new CharacterArrayCharacterIterator(output, 0, outputpos);
			if (regex.match(ci, 0))
				return false;
		}
        ...
        if (max_time > 0.0) {
            if (loop_counter++ > loop_counter_max) {
                ...
                if ((System.currentTimeMillis() - start_time_millis) > max_time_millis)
                    throw new JuggleExceptionDone("Generator_timeout_prefix"+
                                " "+max_time+" "+"Generator_timeout_suffix");
            }
        }
        ...
        if (pos == l) {
            if ((compare_states(pattern_state[0], pattern_state[l]) == 0) && valid_pattern()) {
                if (numflag != 2)
                    print_pattern();
                if (num++ == max_num)
                    throw new JuggleExceptionDone("Generator_spacelimit_prefix"+
                                " "+max_num+" "+"Generator_spacelimit_suffix");
            }
